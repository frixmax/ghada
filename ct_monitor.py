...
# Ton webhook (hardcodé pour simplicité, ou passe par env si tu veux)
...
try:
    ...
    print(f"Erreur chargement domains.txt : {e}")
    targets = set()
...
def on_close(ws, code, msg):
    print(f"Déconnexion ({code}): {msg}. Reconnexion immédiate...")

def heartbeat():
    """Affiche un message toutes les 60s pour garder Render heureux"""
    while True:
        print(f"Heartbeat vivant - {datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S UTC')}")
        time.sleep(60)

# Lancer heartbeat en thread séparé
threading.Thread(target=heartbeat, daemon=True).start()

# Boucle infinie avec reconnexion auto
while True:
    try:
