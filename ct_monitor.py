# ct_monitor.py - CertStream + mini HTTP server pour Render Web Service
...
from flask import Flask, jsonify  # Ajoute Flask
...
# Stats globales (pour la page web)
...
# Route HTTP simple pour Render
@app.route('/')
def home():
    status = "Running" if stats["connected"] else "Reconnecting..."
    return f"""
    <h1>Gungnir CT Monitor</h1>
    <p>Status: {status}</p>
    <p>Last heartbeat: {stats['last_heartbeat']}</p>
    <p>Alertes envoyées: {stats['alerts_sent']}</p>
    <p>Domaines surveillés: {len(targets)}</p>
    """
